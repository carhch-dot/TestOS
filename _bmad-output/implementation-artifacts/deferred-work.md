...
- source_spec: `_bmad-output/implementation-artifacts/spec-4-3-topology-tree.md`
  summary: `TopologyService.tree()`'s multi-query BFS traversal has no transaction/consistent snapshot spanning it — a `Relacion` row that gets deleted (e.g. via a concurrent `DELETE /relations/:id`) between the ring query that admitted one of its endpoints and the later `induceEdges` query can leave a "floating" node in the response: present in `nodes`, with zero edges in `edges` referencing it, `truncated` still `false`. This is a more confusing failure mode than an ordinary stale-read elsewhere in the codebase, because it silently contradicts the endpoint's own "full induced subgraph on the visited node set" guarantee in a structure whose entire purpose is representing connectivity.
  evidence: A correct fix needs the whole traversal (up to ~11 sequential queries at default `TOPOLOGY_MAX_DEPTH=5`) wrapped in one transaction with a consistent snapshot (e.g. Postgres `REPEATABLE READ`) — a new isolation-level pattern this codebase has never used anywhere else, with a real cost (holding a long-running read transaction open across many round trips). Requires two specific operations to race in a narrow window (a relation deletion landing between two of this endpoint's own queries) — revisit if this proves to happen in practice, or if the traversal is ever redesigned to use fewer/batched queries.

- source_spec: `_bmad-output/implementation-artifacts/spec-4-3-topology-tree.md`
  summary: None of `TopologyService`'s Prisma queries specify an `orderBy`, so when `TOPOLOGY_MAX_NODES` truncation actually slices a ring's candidates (`admitCandidates`'s `candidateIds.slice(0, remainingCapacity)`), which specific nodes get admitted versus cut off is not guaranteed stable across two otherwise-identical requests — Postgres does not promise row order for a query without `ORDER BY`. Same non-determinism affects the order of the returned `nodes`/`edges` arrays generally (harmless for the planned XyFlow consumer, which is order-agnostic).
  evidence: Only matters when a graph is large enough to actually hit `TOPOLOGY_MAX_NODES` (default 5000) — an extreme scale for this CMDB's stated scope. Fixing it is cheap in principle (add `orderBy: { id: 'asc' }` to the ring/hydration queries) but wasn't done speculatively for a scenario with no current evidence of mattering; revisit if truncation in practice ever causes a user-visible "the graph looks different every time I reload it" complaint.
