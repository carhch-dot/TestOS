...
| 15 | **Post-review, found via manual testing (not any of the three review layers):** the app failed to boot — `Nest can't resolve dependencies of the JwtAuthGuard (?). Please make sure that the argument JwtService at index [0] is available in the AuditModule module.` | high | Confirmed by actually starting the app. `AuthModule` exported `JwtAuthGuard` (spec-1-8) but never re-exported the `JwtModule` that satisfies its constructor dependency — invisible to every unit test, since each one overrides `JwtAuthGuard` (or `JwtService`) directly rather than resolving it through the real module graph. Fixed: `AuthModule` now also exports `JwtModule`; `AuditModule` imports `AuthModule`. A new `app.module.spec.ts` boots the real `AppModule` (only `PrismaService` mocked, no live DB needed) as a permanent regression guard — verified it fails on the un-fixed code and passes on the fix. |
