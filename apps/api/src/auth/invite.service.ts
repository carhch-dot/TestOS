...
export class InviteService {
  ...
  /**
   * Returns `true` on a successful activation, `false` for every rejection
   * case (unknown/already-used/expired token, or an empty password) — the
  ...
  async activate(token: string, password: string): Promise<boolean> {
    ...
    // Re-check the target Usuario's current status before overwriting its
    // password: a stale-but-still-valid InvitationToken must never be able
    // to silently reset an already-ACTIVE (or DEACTIVATED) user's password
    // with no re-authentication (e.g. a token issued just before a resend
    // race, or replayed against a user activated by another token since).
